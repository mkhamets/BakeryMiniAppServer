...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setupDateInput() {
        ...
        if (dateInput) {
            ...
            // Set min and max dates (works on most platforms)
            ...
            // iOS-specific date validation and enhancement
            enhanceDateInputForIOS(dateInput, today, tomorrow);
    ...
    // iOS-specific date input enhancement
    function enhanceDateInputForIOS(dateInput, today, tomorrow) {
        // Detect iOS
        ...
        if (isIOS) {
            ...
            // Add touch event handlers for better iOS interaction
            ...
            dateInput.addEventListener('change', function() {
                validateAndCorrectDate(this, today, tomorrow);
            });
            
            // Also validate on blur for better iOS support
            dateInput.addEventListener('blur', function() {
                validateAndCorrectDate(this, today, tomorrow);
            });
            ...
            dateInput.setAttribute('data-ios', 'true');
            dateInput.setAttribute('data-today', today.toISOString().split('T')[0]);
            dateInput.setAttribute('data-tomorrow', tomorrow.toISOString().split('T')[0]);
            
            // Create iOS fallback date selector if needed
            createIOSDateFallback(dateInput, today, tomorrow);
        }
    }

    // Create iOS fallback date selector
    function createIOSDateFallback(dateInput, today, tomorrow) {
        // Check if we need the fallback (iOS Safari < 14 or problematic versions)
        const iOSVersion = getIOSVersion();
        const needsFallback = iOSVersion < 14 || /iPad|iPhone/.test(navigator.userAgent);
        
        if (needsFallback) {
            // Create custom date selector
            const fallbackContainer = document.createElement('div');
            fallbackContainer.className = 'ios-date-fallback';
            fallbackContainer.innerHTML = `
                <div class="ios-date-fallback-header">
                    <span>Выберите дату доставки</span>
                </div>
                <div class="ios-date-fallback-options">
                    <button class="ios-date-option" data-date="${today.toISOString().split('T')[0]}">
                        <span class="date-label">Сегодня</span>
                        <span class="date-value">${formatDateForDisplay(today)}</span>
                    </button>
                    <button class="ios-date-option" data-date="${tomorrow.toISOString().split('T')[0]}">
                        <span class="date-label">Завтра</span>
                        <span class="date-value">${formatDateForDisplay(tomorrow)}</span>
                    </button>
                </div>
            `;
            
            // Insert fallback after the date input
            dateInput.parentNode.insertBefore(fallbackContainer, dateInput.nextSibling);
            
            // Hide original input and show fallback
            dateInput.style.display = 'none';
            fallbackContainer.style.display = 'block';
            
            // Add event listeners to fallback options
            const dateOptions = fallbackContainer.querySelectorAll('.ios-date-option');
            dateOptions.forEach(option => {
                option.addEventListener('click', function() {
                    ...
                    // Show feedback
                    showIOSDateFeedback('Дата доставки установлена');
                });
            });
    ...
    // Robust date validation function for iOS
    function validateAndCorrectDate(dateInput, today, tomorrow) {
        ...
        if (isNaN(selectedDate.getTime())) {
            ...
            showIOSDateFeedback('Неверная дата. Установлена сегодняшняя дата.');
            return;
        }
        
        // Validate date range
        if (selectedDate < todayDate || selectedDate > tomorrowDate) {
            // Date out of range, reset to today
            dateInput.value = today.toISOString().split('T')[0];
            
            // Show appropriate feedback message
            if (selectedDate < todayDate) {
                showIOSDateFeedback('Нельзя выбрать прошедшую дату. Установлена сегодняшняя дата.');
            } else {
                showIOSDateFeedback('Можно заказать только на сегодня или завтра. Установлена сегодняшняя дата.');
            }
    ...
    // Show iOS-specific date feedback
    function showIOSDateFeedback(message) {
        // Create or update feedback element
        let feedbackElement = document.getElementById('ios-date-feedback');
        if (!feedbackElement) {
            feedbackElement = document.createElement('div');
            feedbackElement.id = 'ios-date-feedback';
            feedbackElement.className = 'ios-date-feedback';
